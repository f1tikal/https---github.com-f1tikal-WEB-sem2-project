...
if (isset($_GET['logout'])) {
    session_destroy();
    redirect('form_redesign.php');
}

// ---------- Обработка входа (POST на ту же страницу) ----------
$auth_error = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login_action'])) {
    ...
    try {
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            ...
            redirect('form_redesign.php?auth_success=1');
        } else {
...
// ---------- Обработка отправки/редактирования формы ----------
$is_authenticated = isset($_SESSION['user_id']);
$current_user_id = $is_authenticated ? $_SESSION['user_id'] : null;

// Если авторизован, получаем его текущие данные из БД для предзаполнения
$user_data = null;
if ($is_authenticated) {
    try {
        $pdo = new PDO("mysql:host=$db_host;dbname=$db_name;charset=utf8mb4", $db_user, $db_pass);
...
// Обработка POST от основной формы (сохранение/обновление анкеты)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_application'])) {
    ...
    // Если есть ошибки – сохраняем в Cookies и редирект
    if (!empty($errors)) {
    ...
    try {
        ...
        if ($is_authenticated) {
            // Обновление существующей записи (авторизованный пользователь)
            ...
            // Удаляем старые связи языков и вставляем новые
            ...
            redirect('form_redesign.php?registered=1');
        }
    } catch (PDOException $e) {
        $pdo->rollBack();
...
// Для авторизованного пользователя приоритет – данные из БД (user_data)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Определяем, авторизован ли пользователь (есть ли сессия)
    const isAuthenticated = <?= isset($_SESSION['user_id']) ? 'true' : 'false' ?>;
    const userId = <?= $_SESSION['user_id'] ?? 'null' ?>;

    form.addEventListener('submit', async function(e) {
        // Отменяем стандартную отправку, только если fetch поддерживается
        if (window.fetch) {
            e.preventDefault();
        } else {
            return; // фоллбек на обычную отправку
        }
        ...
        try {
            ...
            if (response.ok) {
                if (method === 'POST' && result.login) {
                    ...
                    // Можно также вывести в специальный блок на странице
                } else {
                    alert('Данные обновлены!');
                }
                // Перезагрузить страницу, чтобы отобразить новые данные (или обновить поля через JS)
